...
- `POST /api/alerts/rules` — body in the rules-file format with a required `id`, at most 4 KB. Adds a new saved entry; the editor's Add form uses this (not PUT), so it can't silently overwrite an existing rule. Refused with `409 { errors: { id: "a rule with this id already exists; edit it instead" } }` when any built-in, file or saved entry already has that id; otherwise the same validation, cap and `400 { errors }` as PUT, and `200` with the editor entry followed by `recheck()` and the `rules` notice.
- `PUT /api/alerts/rules/:id` — body in the rules-file format, at most 4 KB. A full rule saves or replaces the entry (PUT replaces an existing entry or adds a new one; the editor's Add form uses POST instead, which refuses a taken id); `{ "disabled": true }` disables a built-in or file rule; an added rule is disabled or enabled by saving it with or without `"disabled": true`. Answers `200` with the editor entry, or `400 { errors: { field: message } }`. An enabled rule whose `for` or `clearAfter` is longer than the raw retention in force is a `400` on that field: "longer than raw retention (2d); raise it on the Settings page first". A full rule saved with `"disabled": true` is only parsed (no metric or retention check): it never runs, so it needn't fit, and editing a disabled rule keeps it disabled. The saved layer is capped at 200 entries. The route `id` must match the body's `id` when the body has one.
