...
- `PUT /api/alerts/rules/:id` — body in the rules-file format, at most 4 KB. A full rule saves or replaces the entry (an unknown `id` adds a rule); `{ "disabled": true }` disables a built-in or file rule; an added rule is disabled or enabled by saving it with or without `"disabled": true`. Answers `200` with the editor entry, or `400 { errors: { field: message } }`. A `for` or `clearAfter` longer than the raw retention in force is a `400` on that field: "longer than raw retention (2d); raise it on the Settings page first". The saved layer is capped at 200 entries. The route `id` must match the body's `id` when the body has one.
- `DELETE /api/alerts/rules/:id` — removes the saved entry (Revert of an edited rule, Enable of a disabled built-in or file rule, or Delete of an added one). `404` when nothing is saved for that id.
...
- `PUT /api/settings` (retention) checks raw retention against the longest look-back of the rules in force now, not a startup snapshot, so it can't be shortened below a rule added in the browser. At startup the same check still refuses a raw retention shorter than the file and built-in rules need; saved rules are checked against it per entry (see [Rule layers](#rule-layers)).
