...
- **Settings** page — device info, retention policy, alert channel configuration
...
- **Retention policy** — the original defaults to \~1 year via RRD rollups; pick a concrete retention/downsampling target now so the `metrics_rollup` schema and the pruning job match what you actually want.
...
- Surface the retention settings on the Settings page so they're tunable without a redeploy.
