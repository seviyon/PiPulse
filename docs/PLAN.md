...
| 5. Alerting & settings | Reimplement the rules engine; make configuration editable | Rule definitions, hysteresis logic, webhook action (e.g. to Apprise); API authentication for write endpoints; a `settings` table (schema migration 3) with `GET/PUT /api/settings`; Settings page with the retention editor | A manufactured threshold breach raises and later cancels correctly; a retention change made in the UI applies within a minute without a restart and survives one; an unauthenticated settings write is rejected |
