...
| 5a. Alerting        | Reimplement the rules engine             | Built. `packages/alerts`: built-in rules merged with an operator `PIPULSE_ALERTS_FILE`, raise/clear hysteresis (`for`/`clearAfter`), a 15 s engine that survives restarts and clock jumps, the `alerts` table (schema migration 4), `GET /api/alerts` and live `alert` messages, tiles coloured from rules, an Alerts page and nav badge | A manufactured breach on the Pi raises an alert that appears live on the dashboard, survives a restart without duplicating, and clears after its clear window |
| 5b. Settings        | Make configuration editable              | API authentication for every write, with optional read protection; a `settings` table (schema migration 5; 3 is `rollup_progress`, 4 is `alerts`) with `GET/PUT /api/settings`; Settings page with the retention editor; rule editing and acknowledging alerts in the browser; notifications (e.g. a webhook to Apprise)                 | A retention change made in the UI applies within a minute without a restart and survives one; an unauthenticated write is rejected                            |
