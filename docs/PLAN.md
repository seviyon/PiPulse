...
| 5b-1. Settings             | Make retention editable, safely          | ✅ Done. Sign-in against an scrypt hash file, in-memory sessions, one auth hook (writes always, reads optionally), `settings` table (migration 5), retention env › saved › default applied live, preview + confirmation before deleting, automatic `VACUUM`, Settings page                                                                                                                                                                                                                                                                                                                                               | Read-only without a password; an unauthenticated write is rejected; a retention change made in the UI applies within a minute without a restart and survives one; a large deletion shrinks the file; read protection shows only the sign-in form |
| 5b-2. Rules in the browser | Make alert rules editable                | ✅ Done (PR #20; two display/restart oddities seen on the Pi are noted as open issues in `CLAUDE.md`). Rules saved from the browser layered over `PIPULSE_ALERTS_FILE` and the built-ins (`createRuleSource()`, read live every check), an engine that closes alerts as `rule_removed`/`rule_changed`, migration 6 (`acknowledged_at`, `rule_hash`), acknowledging alerts, a `swap_io` plugin (pages/s from `/proc/vmstat`) with `swap_heavy` now `swap_io ≥ 250` for 10min, rule/acknowledge API routes, live `rules`/`acknowledged` WebSocket messages, and a rules editor plus Acknowledge buttons on the Alerts page | Tests pass in CI; a rule added, edited, disabled and reverted in the browser on the Pi applies within 15 s without a restart, survives one, and `swap_heavy` no longer stays open on the Pi 2's flat, 81 %-full swap                             |
...
- Automatic compaction (5b-1): SQLite reuses pages freed by deletes but doesn't shrink the file, so after housekeeping PiPulse runs `VACUUM` when free pages are ≥ 25 % of a file over 8 MB and the disk has room for about twice the file, at most once a day, logging the size before and after.
